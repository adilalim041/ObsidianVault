...
### 2026-04-21 — `import os` внутри функции → `UnboundLocalError` на более ранней строке

`vault_qa.answer_from_vault()` падал в production с:
```
WARNING:vault_qa:Gemini failed for vault Q&A: cannot access local variable 'os' where it is not associated with a value
```

Причина: `os` импортирован на module-level (строка 8), но **внутри функции** на строке 179 был повторный `import os`. Python сканирует функцию и помечает `os` как локальную переменную для всей функции. Поэтому `os.getenv("GEMINI_MODEL")` на строке 171 — ДО самого `import` — падал с `UnboundLocalError`.

Fix — удалить `import os` внутри функции (module-level уже есть).

**Lesson**: Python scoping — любое локальное присваивание ИЛИ import имени внутри функции делает имя локальным на всю функцию, независимо от порядка строк. Правило: не дублировать imports внутри функций. Если хочется "lazy import" — вызывать модуль через полный путь (`importlib.import_module`) или переименовывать локальную переменную.

Побочный эффект: Anthropic fallback маскировал этот баг сообщением "Обе AI системы недоступны" — сбивало диагностику. Лучше показывать реальный exception message хотя бы в дебаг-режиме.

### 2026-04-21 — google-genai SDK не автоопределяет mime_type для Telegram .ogg

Phase 2 voice transcription упала с:
```
WARNING voice_transcribe: Unknown mime type: Could not determine the mimetype for your file — please set the `mime_type` argument
```

Telegram voice messages приходят в контейнере `.ogg` (Opus codec). google-genai SDK не детектит MIME автоматически по расширению/magic bytes.

**Fix**: передавать `mime_type="audio/ogg"` явно в `client.files.upload()`. В новом SDK — через `config=UploadFileConfig(mime_type=...)`, в старых — `config={"mime_type": ...}` или `mime_type=...` как kwarg. Лучше пробовать все три варианта через try/except для совместимости.

**Для будущих medja-integrations**: всегда указывать mime_type явно при upload в Gemini Files API, даже если файл имеет стандартное расширение. SDK не обещает автодетекцию.
