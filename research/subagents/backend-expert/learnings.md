...
## 2026-04-21 (Nexus.AI Phase 3 — daily task tracker)

- **Feature-flag pattern for scheduler jobs**: `if os.environ.get("DAILY_TRACKER_ENABLED", "").strip().lower() == "true"` in `register_*_jobs()`. Never register cron jobs unconditionally — allows safe deploy and rollback without code changes.
- **APScheduler reuse, not second instance**: always pass the existing `scheduler` from `core.py` into `register_daily_tracker_jobs(scheduler)`. Creating a second `AsyncIOScheduler` in a module-level singleton would start a second thread and compete with the first. Single scheduler, multiple jobs.
- **`replace_existing=True` + `misfire_grace_time` in APScheduler cron**: prevents duplicate job registration on hot-reload/restart and avoids misfired jobs on startup after Railway deploy. Set `misfire_grace_time=300` (5 min) so brief outages don't skip scheduled runs.
- **Lazy carry-over pattern**: mark yesterday's pending tasks as `carried_over` when user creates a NEW plan, not in a separate cron job. Reason: if user has no plan the next day, there's nothing to carry over. Avoids a 23:30 job that might fire at wrong time due to timezone drift.
- **asyncio.to_thread wrapping sync Gemini inside async handlers**: `_gemini_parse_tasks` and `_gemini_match` are sync functions (blocking network). Always call via `await asyncio.to_thread(_gemini_parse_tasks, prompt)`. Never call sync Gemini/Anthropic directly from async context.
- **Double-fallback AI chain**: Gemini parse → Anthropic parse → raw single task. Three levels of degradation means the user always gets a usable response. The raw fallback is intentionally lossy (1 task from full text) but non-crashing.
- **`asyncio.run()` inside `asyncio.to_thread()` is safe**: `to_thread` runs the function in a thread pool without an event loop. `asyncio.run()` creates a fresh loop in that thread. Calling `asyncio.run()` from async context directly would fail with `RuntimeError: This event loop is already running`.
- **Inline keyboard rebuild after task_done callback**: after marking a task done, rebuild the keyboard with only the REMAINING pending tasks (re-fetch from DB) and `edit_reply_markup`. If no tasks remain, pass `reply_markup=None` to remove the keyboard. Never leave stale buttons.
- **patch.object vs patch("module.attr") in tests**: `patch.object(module, "func_name")` is safer than `patch("module.func_name")` when you have an already-imported reference. The latter requires the string path to be exact and the module to be already in sys.modules. Both work; prefer `patch.object` for imported module attributes.

## 2026-04-21 (Nexus.AI Phase 2 — ideas dual-write + voice transcription)
- **unidecode for Cyrillic slug**: `unidecode("Идея")` produces `"Ideia"` not `"Ideya"` — the output is phonetic approximation, not one-to-one transliteration. Tests must verify structural properties (url-safe, lowercase, no leading/trailing hyphens) rather than exact strings unless you know the exact unidecode output for the specific word.
- **`asyncio.coroutine` removed in Python 3.12**: never use `asyncio.coroutine` as a helper in tests. To mock `asyncio.to_thread`, use `async def fake_to_thread(fn, *a, **kw): return fn(*a, **kw)` and patch with `side_effect=fake_to_thread`. Works in any Python ≥ 3.7.
- **Dual-write pattern — vault as secondary store**: primary write (Supabase) always first, vault write always in `try/except` with `logger.warning` on failure. Return `None` instead of raising. Caller checks `if vault_result` before showing URL button. This pattern keeps the bot functional even if GitHub API is down.
- **GitHub Contents API response shape**: PUT returns `{"content": {"html_url": "...", "sha": "..."}, "commit": {...}}`. Always extract `result["content"]["html_url"]`, not top-level `result["html_url"]`.
- **vault_github.py retry**: exponential backoff only for 429/502/503/504 (transient). 409 (sha conflict) = re-fetch sha + one more try (not a generic retry). Other 4xx = fail immediately. `httpx.TransportError` (network) = same backoff as 429.
- **Voice duration guard before file download**: check `message.voice.duration < 3` BEFORE `bot.get_file()` + `bot.download_file()` to avoid wasting Telegram API calls + disk I/O on useless recordings.
- **Gemini Files API cleanup pattern**: always wrap `client.files.delete()` in `finally` block, but never re-raise from finally. Use `try/except` inside finally. Otherwise cleanup error masks the real transcription error.
- **SessionStart hook to inject recent context**: adding project-specific recent data (last N ideas) to vault-autoload.py `build_context()` is the right extension point. Read at most 1KB per file (frontmatter only). Sort filenames descending to get newest first. Hard limit to max_count=5 to stay within 2KB context budget.

## 2026-04-22 (Phase 3 followup — borderline GET/POST cases)
- **`_req` → `req` rename for userClient access**: handlers using `_req` (underscore) can't access `req.userClient`. Pattern: rename to `req`, then add `const db = req.userClient ?? supabase;` as first line. Applies to GET /sessions (was `_req`).
- **`db` placement in send handler with nested try blocks**: POST /sessions/:sid/send has a double-try structure (outer: send, inner: save+AI pipeline). `const db = req.userClient ?? supabase;` goes BEFORE the outer `try`, so all inner scopes can reference it without re-declaration.
- **Inline deep-nested supabase.from in AI pipeline**: the `messages.update({ dialog_session_id })` inside `getOrCreateDialogSession` callback is user-facing data (links message to dialog) — must use `db`, not `supabase`. `trackResponseTime` and `getOrCreateDialogSession` themselves stay on service_role (internal helpers, no user scope needed).
- **Final grep gate for single endpoint**: after migrating an endpoint, run `awk 'NR>=START && NR<=END' routes.js | grep "supabase\.from"` to confirm zero remaining bare calls in that handler's line range. Prevents silent misses.

## 2026-04-22 (Phase 3 shadow-scope — upsertChatTags + write endpoints)
- **Helper db-param pattern with backward-compat default**: `export async function upsertChatTags(remoteJid, opts = {}, db = supabase)` — adding `db` as last positional param with `= supabase` default keeps all existing callsites (aiWorker.js, admin backfill, internal ingestion) working with service_role without touching them. Only user-facing handlers inject `req.userClient ?? supabase`. This avoids the need to grep all callsites before migration — default is always safe.
- **Shadow-scope discovery pattern**: after Phase 3 writes, senior-reviewer found 6 more same-class writes (mute, hide, tags, confirm-tag, messages/read, read-all) that weren't in the declared scope but had the same RISK-1 (horizontal privilege escalation in SaaS). Lesson: scope declaration for userClient migration should include ALL chats/messages/contacts_crm/chat_ai writes, not just the ones named in the ticket. Use `grep -n "supabase.from\|await supabase" routes.js` as a final diff to catch stragglers.
- **Destructive write (DELETE) through userClient is correct**: `/chats/:phone/hide` does `DELETE FROM messages` — routing it through `req.userClient ?? supabase` is the right call. The user's RLS policy controls what rows they can delete. Never leave destructive ops on service_role just because they "feel dangerous" — that's the worst case for horizontal escalation.
- **`/reports/send` service_role intent comment placement**: put the SERVICE-ROLE INTENTIONAL comment at two points — (1) before the idempotency SELECT (dedup must survive JWT expiry between upload and audit), (2) before the `insertManagerReport`/`markChatAiReportSent` block (audit trail invariant). Two separate comments is correct because the reasons differ: one is timing/expiry, the other is audit completeness.
- **Promise.all with db param**: in read-all, the `Promise.all([db.from(...), db.from(...)])` pattern works correctly — both calls use the same `db` instance (userClient or supabase). No need for per-call db resolution.

## 2026-04-21 (Nexus.AI semantic memory — pgvector + Gemini embeddings)
- **Two-table pgvector split**: `nexus_conversations` (lightweight text rows) + `nexus_conversation_embeddings` (vector(768)). Never mix vectors into the main message table — bloats row width from ~100 bytes to ~3100 bytes, hurts frequent non-vector reads (get_recent, save_message).
- **HNSW index params for 768-dim**: `m=16, ef_construction=64` — balanced accuracy/memory for personal-scale datasets. For production scale: increase m to 32-64.
- **Fire-and-forget embedding pattern**: INSERT synchronously (returns conv_id immediately), then `asyncio.create_task(_embed_async(conv_id, content))`. Handler never waits on embedding. If embedding fails → log warn, message still accessible via get_recent. Never block UX on ML inference.
- **supabase-py is synchronous** — all supabase-py calls must be wrapped in `asyncio.to_thread()` inside async functions. Never `await supabase.table(...)` directly — supabase-py client is not async despite aiogram being async.
- **RPC with vector param**: `sb.rpc("func_name", {"p_query_embedding": list_of_floats, ...}).execute()` — pass the embedding as a plain Python list, supabase-py serializes to JSON array, Postgres casts to `vector`. No special serialization needed.
- **Gemini text-embedding-004 response shape**: `response.embeddings[0].values` — a list of floats. Note: `response.embeddings` is a list even for single-content requests. Always index `[0]`.
- **Retry pattern for embedding**: only retry on `429` or `500` (rate limit / server error). Other errors (auth, model not found) are non-retriable — immediate raise. Check `str(exc).lower()` for "429", "500", "quota", "rate" as strings since SDK exception hierarchy varies.
- **Content truncation before embed**: truncate to 8000 chars (~2048 tokens Russian) BEFORE calling embed API. Apply truncation only for embedding, not for DB storage. Store full content in conversations table.
- **Module isolation in tests**: always delete semantic_memory, memory, config from sys.modules in autouse fixture. Module-level singletons (_genai_client, _supabase_client) survive between tests otherwise.
- **Chainable mock for supabase-py**: `chain = MagicMock(); chain.insert.return_value = chain; chain.select.return_value = chain` etc. All methods return the same `chain` object. `chain.execute = MagicMock(return_value=MagicMock(data=[...]))`. Then `mock_sb.table.return_value = chain; mock_sb.rpc.return_value = chain`.
- **asyncio.create_task naming**: always pass `name=` kwarg to `create_task` for debuggability: `asyncio.create_task(coro, name=f"embed_conv_{conv_id}")`. Visible in `asyncio.all_tasks()` output.
- **RLS DISABLE vs ENABLE+no-policy**: for service-only tables (no user JWT access), always `ALTER TABLE t DISABLE ROW LEVEL SECURITY`. "Enable+no policy" is the silent-deny trap that returns `data: []` indistinguishable from empty table. Confirmed again for nexus_conversations.

## 2026-04-21 (Nexus.AI Phase 1B — semantic memory runtime integration)
- **Semantic retrieval latency budget**: Gemini embed query ~200-400ms + Supabase RPC ~50-100ms = ~300-500ms per message. Since `asyncio.to_thread()` keeps event loop free and "Думаю..." spinner already shows, UX is unaffected. Always show a thinking indicator before retrieval, never block on ML before responding.
- **Graceful degradation pattern for memory layer**: every call to Supabase/Gemini in the message handler must be wrapped in try/except that logs `WARNING` and sets a safe default (`semantic_ctx = []`). Never let a failed memory operation crash the bot. The bot's primary function (responding) ALWAYS takes priority over memory operations.
- **Deduplicate semantic results against in-memory history**: `search_similar` can return messages that are already in the last 8 in-memory turns. Always deduplicate before injecting into prompt. Use exact content match (stripped). O(n*m) is fine for 8x3 comparison.
- **Prompt injection mitigation with structured output**: even if adversarial content enters semantic context, Gemini's `response_schema=IntentResponse` with `Literal` enum for intent field neutralizes most attacks — invalid intents are rejected at schema validation level. Still label semantic context clearly as "past memories" in prompt, not as instructions.
- **`/clear` command extension pattern**: when extending slash commands with sub-parameters (e.g. `/clear all`), parse via `message.text.split(maxsplit=1)`. `args[1]` is the optional parameter. No separate command registration needed — same `Command("clear")` handler handles both cases.
- **`count="exact"` for Supabase stats queries**: use `.select("id", count="exact")` to get row count without fetching all rows. Result in `response.count` attribute. Always guard with `hasattr(resp, "count") and resp.count is not None` — older supabase-py versions may not support it and fall back to `len(resp.data)`.
- **DELETE in_ for related table cleanup**: `sb.table("nexus_conversation_embeddings").delete().in_("conversation_id", list_of_ids).execute()`. Must fetch IDs from main table first (SELECT id WHERE user_id=X), then DELETE embeddings with those IDs, then DELETE from main table. FK order matters.
- **`_format_semantic_context` helper**: when building LLM prompts with retrieved memories, format each item with role, relative time ("Nд назад"), and content truncated to 300 chars. This prevents semantic context from dominating the prompt. Place this block BETWEEN global_context and chat_history, not at top (system-level rules take priority).

## 2026-04-21 (W1.1 Phase 2 Day 3 — 7 call/contact/reports helpers)
- **Хелперы с options-объектом** (`getCallsBySession`, `getCallsByChat`): `db` добавляется внутрь деструктурируемого объекта как `{ limit, offset, db = supabase }`. Это сохраняет backward-compat — позиционные callers без `db` продолжают работать без изменений.
- **Хелперы с positional параметрами** (`getMissedCallsCount`, `getCallsKpi`, `getContacts`, `getChatAiById`, `getActiveSessions`): `db = supabase` добавляется последним позиционным параметром. Ни один существующий caller не передаёт `db` → никакого слома.
- **Fallback-ветка в `getActiveSessions`**: хелпер имеет fallback query `supabase.from(...)` при ошибке PGRST116. Оба запроса (primary + fallback) должны использовать один и тот же `db` — иначе fallback возвращает данные через другой client. Заменять только `.from()` недостаточно — нужно пройти весь хелпер до конца.
- **`getMissedCallsCount` без callsite в routes.js**: хелпер экспортирован, но не вызывается через REST пока. `db` параметр добавляется превентивно — иначе при добавлении эндпоинта придётся делать повторный edit хелпера.
- **`getActiveSessions` в `/reports/send`**: `db` объявляется выше блока `try` (после input validation) — это позволяет передать его в `getActiveSessions(db)` внутри `try`. Порядок: validation → `const db = req.userClient ?? supabase` → `try { ...queries }`.
- **Итоговая модель**: 7 хелперов = 7 коммитов. Каждый = правка queries.js + правка routes.js (где есть callsite). Syntax check `node --check` между каждым коммитом — обязательный gate.

## 2026-04-21 (GFM anchor generation for citations validator)
- GitHub Flavored Markdown anchor algorithm: lowercase → remove all punctuation except `-` and `_` → collapse whitespace to `-`. Key: `##  Rules of Hooks (React 18)` → `rules-of-hooks-react-18`. Strip leading `#` chars before processing raw heading lines.
- JSONL validation pattern with Node stdlib only: `readFile` → `split('\n').filter(Boolean)` → `JSON.parse` per line with try/catch skip on malformed. Cache file reads with a Map (slug → anchors Set) to avoid re-reading same learnings.md per slug across many citations.
- GFM does NOT collapse consecutive hyphens — `p-retry -- aborterror` stays as is. Don't post-process runs of `-`.
- Real-world finding (2026-04-21): agent_runs.jsonl citations use kebab-slug format (`backend-expert#graphile-worker-port-5432`) but learnings.md headings use date-topic format (`## 2026-04-21 (Topic)`). hallucination_rate=1.0 on first run — correct diagnosis, not a bug. The citation convention and heading convention are mismatched system-wide.

## 2026-04-21 (Supabase RLS — DISABLE vs permissive policy for service tables)
- Two correct postures for tables that must never be read via user JWT: `ALTER TABLE t DISABLE ROW LEVEL SECURITY` (not "enable with no policy" which is the silent-deny trap). Applies to service-only tables like `auth_state`, `session_lock` — Baileys / leader-election tables that only ever use the service_role key.
- Silent-deny trap: RLS enabled + zero policies = `supabase-js` returns `data: []` without an error. Indistinguishable from "table is empty". Worst failure mode for debugging Phase 2 userClient migration.
- RLS migration pattern (Omoikiri 0002/0003): single DO-block with `text[]` array, FOREACH, `IF NOT EXISTS pg_tables` portability guard, `DROP POLICY IF EXISTS` for idempotency, then `CREATE POLICY authenticated_all FOR ALL TO authenticated USING(true) WITH CHECK(true)`. Service tables get separate DO-blocks with `DISABLE ROW LEVEL SECURITY`.
- Post-run verification: always include commented probe queries in the migration file itself — (1) confirm policies exist, (2) confirm service tables have `rowsecurity=false`, (3) CI regression guard: `SELECT tablename FROM pg_tables WHERE schemaname='public' AND rowsecurity=true AND tablename NOT IN (SELECT tablename FROM pg_policies WHERE schemaname='public')` → must return 0 rows.

## 2026-04-21 (Vercel serverless + GitHub Contents API write endpoint)
- `crypto.timingSafeEqual` requires equal-length Buffers. Pattern: hash both sides with SHA-256 first (`createHash('sha256').update(s).digest()`), then compare — Buffers are always 32 bytes. This prevents the length oracle while still being timing-safe. Never compare raw strings of different lengths directly.
- GitHub Contents API write flow confirmed (learning from vault_github.js 2026-04-20): GET first to get sha, then PUT with sha. Skipping sha on existing file returns 409 Conflict. On NEW file (dedup check returned 404), PUT without sha = create. Same helper works for both.
- Vercel Serverless Function TypeScript: use `export const config = { runtime: 'nodejs' }` (NOT 'edge') when you need Node.js builtins (`node:crypto`, `Buffer`). Edge runtime does NOT have these.
- `import type { VercelRequest, VercelResponse } from '@vercel/node'` — types only, goes in devDependencies, zero runtime overhead. Zod already installed as transitive dep in many React projects — check before adding to deps.
- Zod v4 (4.x) is backward-compatible with v3 classic API via `import { z } from 'zod'` which re-exports `zod/v4/classic/external.js`. `.safeParse()`, `.enum()`, `.nullable()`, `.regex()` — all identical. No migration needed.
- AbortController timeout pattern for fetch: `const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), MS); try { return await fetch(url, { signal: controller.signal }) } finally { clearTimeout(timer) }`. The `finally` is critical — without it, the timer leaks on success and will abort a future unrelated request.
- Vercel Functions + SPA rewrites coexist: put `"functions"` block before `"rewrites"` in vercel.json. The `/api/*` routes are handled by Functions FIRST, the `/(.*) → /index.html` rewrite only catches everything else. No conflict.
- DRY_RUN env pattern for write-endpoints: `if (process.env.DRY_RUN === 'true') return earlyResponse`. Makes the endpoint fully testable without real I/O. The fake id should include "dry-run" substring so logs are distinguishable.
- Non-fatal partial success pattern: queue write (primary) succeeds → candidate card update (secondary) fails → log warn, return 201 with `candidate_status_updated: false`. Do NOT attempt rollback of queue write — rollback complexity > partial inconsistency for this use case.

## 2026-04-22 (W1.1 Phase 3 — write-path endpoints + chatsCache P-2)
- **Write-path pattern** (10 коммитов): `const db = req.userClient ?? supabase;` ставится сразу после input validation (но до `try {`), затем все `supabase.from(...)` внутри handler-а заменяются на `db.from(...)`. Для аватар-endpoint — `db` объявляется после Cloudinary upload, т.к. до этого нет Supabase запросов.
- **POST /crm/deal-stage — три запроса, один `db`**: SELECT (найти latest), UPDATE (если есть), INSERT (если нет) — все три через один `db`, объявленный до `try`. Не объявлять `db` три раза внутри каждой ветки — это ошибка.
- **DELETE с pre-check SELECT**: оба запроса (SELECT для ownership-verify + DELETE) используют один `db` — иначе SELECT пройдёт через RLS, а DELETE через service_role, что допускает удаление чужих записей.
- **chatsCache P-2 (drop db===supabase identity check)**: `cacheUserId = db === supabase ? '__service__' : (userId ?? 'anon')` заменено на `cacheUserId = userId ?? '__service__'`. Callsites в routes.js: `req.user?.id ?? null` → `req.user?.id ?? '__service__'`. Убрать `'anon'` как fallback — `'__service__'` безопаснее: над-изолирует в худшем случае, а `'anon'` может под-изолировать.
- **`/reports/preview` oversight pattern**: handler уже имел `const db = req.userClient ?? supabase` но три inline `supabase.from(...)` были добавлены позже и не мигрированы. Grep `supabase\.from` внутри handler после добавления `db` — стандартный финальный проверочный шаг.
- **Scope confirmed**: AI worker, `/admin/*`, session-lifecycle writes, `/reports/send` + idempotency-check — все осознанно остаются на service_role. Не мигрировать.
- **Итог Phase 3**: 10 коммитов, 9 pushей в main, Railway передеплоился автоматически. Файлы: `src/api/routes.js`, `src/storage/queries.js`.

## 2026-04-21 (analyticsCache composite key — avoid db===supabase identity check anti-pattern)
- When fixing cross-tenant cache bleed, userId must be passed **explicitly** from the HTTP handler — NOT inferred via `db === supabase` identity check. Reason: `getUserClient(jwt)` returns `null` on missing env-var or malformed JWT → fallback `req.userClient ?? supabase` → `db === supabase` resolves to `true` → anon/broken-auth requests silently land in `__service__` cache bucket, bypassing the fix entirely.
- Correct pattern in handler: `const userId = req.user?.id ?? '__service__';` then pass to cache-key builder as explicit arg. No reference comparison needed.
- `'__service__'` is the correct fallback (not `'anon'`) for x-api-key / worker paths where `req.user` is absent. `'anon'` should be reserved for back-compat callers that genuinely don't have auth context and don't go through RLS — if you can't tell what a caller is, `'__service__'` is safer (worst case it over-isolates; `'anon'` could under-isolate).
- `invalidateAnalyticsCache()` can remain a full `.clear()` — evicting all user buckets on any write mutation is conservative but correct. The alternative (per-user invalidation) requires tracking which buckets are warm, adds complexity, and the 2-minute TTL makes it low priority.
- Audit mirror check: whenever you fix a tenant-bleed in one in-memory cache, immediately grep for all other `new Map()` declarations in the same file and verify their keys include a user dimension if they store RLS-filtered data.

## 2026-04-21 (chatsCache tenant-bleed fix — composite cache key)
- When a module-level Map cache is keyed only by `sessionId`, a service_role call populates it; a subsequent user-scoped call with the same sessionId (but a different Supabase client with RLS) hits the warm entry and receives rows it would not have access to directly. Fix: composite key `${sessionId}::${userId}` where userId is `'__service__'` when `db === supabase` (the module-level serviceClient), and `req.user?.id ?? 'anon'` for userClient callers. The `db === supabase` identity check is cheap and works because the serviceClient is a module singleton — no extra async RPC needed (avoids option b).
- 3rd-param `userId = null` default keeps backward-compat: any internal caller that doesn't pass userId gets `cacheKey = "${sessionId}::anon"` which is isolated from both service and specific-user entries.
- Pattern for future caches in multi-client contexts: every Map key that holds RLS-filtered data must include a userId (or role) dimension, not just a resource dimension. Single-tenant: only `sessionId` is fine. Multi-tenant / mixed-privilege: always composite.

## 2026-04-21 (W1.1 Phase 2 — RLS-aware dbClient threading in queries.js)
- Pattern: thread a `db = supabase` default parameter through helper chain so callers can pass `req.userClient` (Supabase anon-key client bound to user JWT) for RLS-filtered reads. Default keeps backward-compat for all internal callers (workers, cron, admin endpoints) that never pass `db`.
- In Express handlers: `const db = req.userClient ?? supabase;` — assign once at handler entry, pass through to every query call. Never assign `req.userClient` as a module-level import.
- In-memory process-level caches (Map keyed by sessionId) are only safe in single-tenant deploys. When RLS filters by user_id and cache is keyed only by sessionId, one user could serve another user's cached rows. TODO comment on cache line; fix = add userId to cache key. Mark clearly so it's not missed on SaaS migration.
- Grep verification method on Windows (CRLF line endings): `grep -E` patterns with `\n` don't work. Use `awk 'NR>=X && NR<=Y' file | grep "pattern"` to check specific line ranges, or plain `grep -n "literal" file`. Alternatively use Read tool to visually verify.
- Syntax check gate after any edit to query layer: `node --check src/storage/queries.js && node --check src/api/routes.js`. The import smoke-test (`node --input-type=module -e "import(...)"`) will fail on startup guards (SUPABASE_SERVICE_ROLE_KEY required) — that's expected in dev without .env. Use static signature grep instead to confirm function exports.

## 2026-04-21 (W1.1 Phase 2 Day 2 — ещё 6 endpoints, итого 13)
- **Хелперы, потребовавшие threading (queries.js):** `listManagerReports` (добавлен `db` в деструктурированный options-объект), `getLinkedSessions` (новый 2-й параметр `db = supabase`), `getChatTags` (новый 2-й параметр `db = supabase`), `getUnifiedMessages` (новый 4-й параметр `db = supabase`).
- **Неочевидное пересечение 1 — `getChatTags` внутри `getLinkedSessions`:** `getLinkedSessions` вызывает `getChatTags(remoteJid)` через `Promise.all`. Без прокидывания `db` теги приходили через service_role даже когда всё остальное уже шло через userClient. Паттерн идентичен `getChatTagsByJids` в `/crm/funnel` (Day 1): всегда передавай `db` во все внутренние read-хелперы, не только в прямые `.from()` вызовы.
- **Неочевидное пересечение 2 — `listManagerReports` с options-объектом:** helper принимает `{ sessionId, dateFrom, dateTo, limit }`. Добавление `db` в деструктуру — `{ ..., db = supabase }` — сохраняет backward-compat: все внутренние вызовы (worker, cron) не передают `db` и получают serviceClient по умолчанию. Не нужно менять сигнатуру на positional параметр.
- **Неочевидное пересечение 3 — `/reports` читает три таблицы:** основные данные через `listManagerReports(db)`, потом ещё два inline-запроса на `session_config` и `chats` для name-resolution. Если пропустить inline-запросы, session_config придёт через service_role — пользователь увидел бы имена сессий без RLS-фильтрации. Все три читателя должны использовать один `db`.
- **Неочевидное пересечение 4 — `/reports/monthly` мешает три таблицы:** `messages` (трафик), `tasks` (статистика), `chat_ai` (AI-анализ). Все три — user-facing данные, все три мигрированы. Аналитика не имеет смысла если `messages` идут через userClient, а `tasks` через service_role.
- **`getUnifiedMessages` — 4-й positional параметр:** сигнатура `(remoteJid, limit, offset, db)` — `db` добавлен последним, не ломает ни одного существующего вызова (callers передают 3 позиционных аргумента максимум). Проверено grep-ом.
- **Коммиты:** 6 коммитов, запушены в main (b455551..d5c8f97). Phase 2 RISK-1 закрыта полностью для всех GET reader-endpoint'ов.

## 2026-04-21 (W1.1 Phase 2 Day 1 — 7 endpoints mигрированы)
- **Хелперы, потребовавшие threading (queries.js):** только `getMessages` — добавлен 5-й параметр `db = supabase`. `applyChatTagsOverlay` и `getChatTagsByJids` уже имели `db` параметр из предыдущего коммита a13690c.
- **Инлайн-запросы в routes.js:** `/messages/search` (4 вызова), `/analytics/summary` (5 вызовов через `let query = supabase...` + `sessionFilter(query)` паттерн), `/analytics/chats-by-filter` (3 вызова + передача db в getChatTagsByJids), `/crm/funnel` (4 вызова + batch loop + getChatTagsByJids).
- **`sessionFilter` паттерн в analytics/summary:** замыкание `(query) => sessionId ? query.eq(...) : query` не использует `supabase` напрямую — оно только добавляет `.eq()` к уже-созданному query. Поэтому достаточно заменить исходный `supabase.from(...)` на `db.from(...)`, замыкание работает без изменений.
- **Неочевидное пересечение:** `getChatTagsByJids` вызывается внутри `/crm/funnel` через `Promise.all([chatQuery, getChatTagsByJids(jids)])` — нужно явно передавать `db` как второй аргумент, иначе tags возвращаются через service_role пока все остальные данные через userClient (молчаливое несоответствие).
- **Endpoints НЕ трогать:** POST/PATCH/DELETE на contacts/:phone — это writer-пути, не reader. Worker и cron пути тоже остаются на serviceClient по замыслу.
- **Коммиты:** один endpoint = один коммит, формат `W1.1 Phase 2: <path> via userClient`. 7 коммитов, все запушены в main, Railway передеплоит автоматически.

## 2026-04-20 (Nexus.AI /status — aiogram v3 callbacks + rich status UX)
- `@router.callback_query(F.data == "x")` and `@router.callback_query(F.data.startswith("x"))` — both are identity-decorator patterns. In test stubs, the `_Router` class needs both `.message()` and `.callback_query()` methods returning `lambda fn: fn`, otherwise handlers are replaced by MagicMock at import time.
- `InlineKeyboardMarkup(inline_keyboard=[[btn, btn], [btn, btn, btn]])` — aiogram v3 takes a list-of-lists, each inner list is one row. Never pass flat list.
- `InlineKeyboardButton(text="...", callback_data="...")` — keyword-only args in aiogram v3. No positional args.
- Callback dispatch pattern: always `await callback.answer()` at the end (stops the spinner). On error: `await callback.answer("Ошибка", show_alert=True)` + log + return.
- `F.data.startswith("prefix:")` filter: to extract the suffix use `callback.data.removeprefix("prefix:")` (Python 3.9+). Cleaner than split/slice.
- `_fetch_all_for_status()` — use `asyncio.ensure_future()` + `asyncio.gather()` to run status fetch + queue count concurrently. Saves one sequential network round-trip.
- Queue count on the keyboard button: fresh fetch on every `/status` call (no cache). The button label `📬 Очередь (N)` reflects current state at render time. This is correct for low-frequency commands — no need to cache.
- `F` stub for aiogram tests: `F.data == "x"` and `F.data.startswith("x")` need to return truthy MagicMocks without raising. A `_FDataStub` inner class with `__eq__` and `startswith` returning `MagicMock()` works cleanly.
- `asyncio.ensure_future()` inside `async def` requires a running event loop (it uses the current loop). Safe inside coroutines, NOT at module level.

## 2026-04-20 (Omoikiri heartbeat — vault_github.js ESM wiring)
- `setInterval().unref()` pattern — critical for background timers that should not block graceful shutdown. Without `.unref()` the process hangs until the timer fires on SIGTERM. All "optional" background jobs (heartbeat, keep-alive) must use `.unref()`.
- Supabase health as failoverQueue proxy: `failoverQueue.length === 0` is a clean `supabase_ok` signal — no extra DB roundtrip needed. When Supabase is down, messages queue up; when it recovers, queue drains. Heartbeat readers see the truth without extra load.
- GitHub Contents API write flow: (1) GET to read current sha (returns 404 if new file), (2) PUT with optional `sha`. Skipping the GET and PUTting without sha on an existing file returns 409 Conflict. The vault_github.js helper handles this automatically.
- `_touchLastMessageProcessed()` placement: set inside `saveMessage()` on `!error` path (successful DB upsert), not in messageHandler. Keeps telemetry co-located with the operation it measures; naturally excludes failover-queued messages.
- Avoid name collisions with existing exports: `startHeartbeat`/`stopHeartbeat` are already exported from `src/storage/sessionLock.js`. Named the vault variants `startVaultHeartbeat`/`stopVaultHeartbeat` to avoid shadowing in index.js.
- node:test module cache: dynamic `import()` in `before()` returns the cached module on the second suite. For isolated state, test public API behavior rather than internal variables. Or export a reset function if necessary.

## 2026-04-20 (News.AI heartbeat — vault_github.js CJS adaptation)
- vault_github.js из system/lib — ESM (export). При копировании в CJS-сервис (без `"type":"module"`) нужна замена `export async function` → `async function` + `module.exports = { ... }` в конце. Остальное идентично — Buffer.from/to base64, PUT с sha lookup, fetch (Node 18+).
- Heartbeat pattern для Railway-сервиса: `if (!VAULT_WRITE_TOKEN) { logger.warn(...); return; }` в startHeartbeat — warn once и выход без регистрации интервала. Так local dev работает без токена без любых exceptions.
- Метрики из Supabase: `articles_today` = COUNT WHERE status='published' AND published_at >= today UTC 00:00:00. `queue_depth` = COUNT WHERE status IN ('raw','classified','ready','processing'). `last_article_published_at` = ORDER BY published_at DESC LIMIT 1. Все три — разные запросы, каждый best-effort с индивидуальным try/catch.
- Честный null лучше сфабрикованного числа: `error_rate_1h: null` — явно задокументировано "добавить когда Sentry wired". Не создаём счётчик ради счётчика.
- Vitest unit-тест для heartbeat: mock Supabase как fluent chain (from→select→eq→order→limit→maybeSingle/then all returning the next queued response). vi.stubGlobal('fetch') для перехвата vault write. Тест "fetch fails → no throw" обязателен для fire-and-forget модулей.
- Место wiring в server.js: `app.listen(PORT, callback)` → внутри callback после `startScheduler()`. Так heartbeat стартует только после успешного старта сервера, а не в process startup до listen.

---

## 2026-04-20 (Nexus.AI vault bus commands — aiogram v3 + pytest stubs)

- `@router.message(Command("x"))` — это декоратор. Если в тестах `Router = MagicMock`, декоратор заменяет функцию MagicMock'ом. Правильный stub: `class _Router` с `def message(self, *a, **kw): return lambda fn: fn` — identity decorator, возвращает оригинальную функцию.
- `pytest-asyncio` в STRICT mode требует явного `@pytest.mark.asyncio` на каждом async тесте — автодетект отключён.
- Конфликт команд в aiogram v3: если два роутера регистрируют `Command("status")`, побеждает тот что включён первым через `dp.include_router()`. Решение: переименовать старую команду (в данном случае `/project_status`) и зарегистрировать новый роутер вторым.
- `httpx.AsyncClient(timeout=20.0)` — AsyncClient принимает keyword args, stub-класс для тестов должен иметь `def __init__(self, **kwargs)`.
- `read_vault_raw` из vault_github.py — unauthenticated CDN read для публичного репо. Используй для status/*.json (high-frequency polling), а `list_vault` + authenticated `read_vault` — для candidates (requires token).
- Импорты внутри функций (`import asyncio; import httpx`) мешают патчингу через `patch("module.httpx.AsyncClient", ...)`. Лучше делать top-level imports и патчить через `patch.object(module, "httpx", ...)` или через полный путь.

---

## 2026-04-20 (Spam tag + AI guard для сотрудник/спам)
- wa-bridge CUSTOMER_TYPE_TAG mapping: добавили `spam: 'спам'` — теперь AI может напрямую тегировать спам-чаты, вместо того чтобы сваливать их в 'неизвестно'. Это важно, потому что `'спам'` попадает в `AI_AUTO_TAGS` автоматически через `new Set(Object.values(CUSTOMER_TYPE_TAG))` — не надо добавлять вручную.
- runDailyAnalysis skip-guard pattern: `getChatTagsByJids(allJids)` батч ДО цикла. Если упал — fail-open (пустой Map, продолжаем). `SKIP_TAGS = new Set(['сотрудник','спам'])`. Проверка: `chatTags.some(t => SKIP_TAGS.has(t))` → `skipped++, continue`. Счётчик `skipped` уже существующий — не нужен новый.
- classifyUntaggedChats skip-guard: аналогичный `CLASSIFY_SKIP_TAGS` в `.filter()` перед батч-загрузкой сообщений. Важно: этот Set отдельный от `SKIP_TAGS` в runDailyAnalysis (разные замыкания). Если сотрудник поставлен ошибочно — пользователь убирает руками, AI не помощник.
- Spam threshold в промпте: высокий порог уверенности достигается через список КОНКРЕТНЫХ примеров ("Предлагаем продвижение в Instagram", "Кредиты", "SEO") + explicit "НИКОГДА для вопросов о продукции" + explicit "если сомневаешься → unknown". Три anchor-а надёжнее одного абстрактного правила.
- CLASSIFY_PROMPT (Haiku) тоже обновлён — оба пайплайна (Sonnet daily + Haiku classify) должны знать про spam. Иначе Haiku вернёт spam → `CUSTOMER_TYPE_TAG['spam']` вернёт 'спам', что правильно, но промпт Haiku не объяснял критерии → риск ложных срабатываний.
- Fail-open правило для tag-filter: если `getChatTagsByJids` падает → dailyTagMap остаётся пустым объектом → `chatEntry?.tags || []` даёт пустой массив → `.some(t => SKIP_TAGS.has(t))` возвращает false → диалог НЕ пропускается. Правильно: лучше переплатить токены, чем пропустить реального клиента.

## 2026-04-20 (Nexus.AI heartbeat — asyncio task + Python vault_github.py)
- Python heartbeat wiring pattern: `asyncio.create_task(start_heartbeat_task(), name="nexus_heartbeat")` INSIDE `main()` after `scheduler.start()`, BEFORE `dp.start_polling(bot)`. The event loop must already be running (we're inside `async def main`). NOT in module-level or in `asyncio.run()` callback — the task must be created from within a running loop.
- Token missing guard in Python: `if not os.environ.get("VAULT_WRITE_TOKEN"): logger.warning(...); return` at loop start. Exits the coroutine immediately, no loop, no repeated warnings. Set `_token_missing_warned = True` to signal tests. This is the Python equivalent of the JS `if (!VAULT_WRITE_TOKEN) { logger.warn; return; }` pattern.
- Swallowing exceptions in fire-and-forget tasks: catch `Exception` (broad) but handle `RuntimeError` about tokens separately for better log signal. The key invariant: heartbeat exception must NEVER propagate to the caller or cancel the parent task. Use `logger.warning(...)` not `logger.exception(...)` to avoid noisy tracebacks in production logs for expected network failures.
- In-memory metrics without new DB queries: `notify_command(user_id)` and `notify_mempalace_result(ok)` are pure in-memory hooks called from existing handlers. No new DB queries, no APScheduler jobs. Active users = `len(_active_user_ids)` where the set is filled by notify_command. Uptime = `time.monotonic() - _start_time` (process-relative, no wall clock drift issues).
- httpx was missing from Nexus requirements.txt — added `httpx>=0.27.0` to both requirements.txt and requirements-cloud.txt. The vault_github.py reference helper requires it.
- pytest-asyncio mode: in asyncio strict mode, `@pytest.mark.asyncio` is required on each async test. Mock of module-level state requires `autouse=True` fixture that resets module attributes directly (`import heartbeat; heartbeat._field = value`). Don't use `importlib.reload()` — it breaks the patch.dict sys.modules approach.

## 2026-04-20 (Analytics employee filter)
- wa-bridge analytics exclusion pattern: load relevant jids with `.overlaps('tags', ['клиент','партнёр'])` once at endpoint start, build a `Set<string>`, then JS-filter all result arrays. Simpler than PostgREST JOIN — avoids `.in()` on large sets and works with existing query structure.
- `shouldSkipAnalytics` guard for responseTracker: skip `сотрудник` always; skip `неизвестно`/empty tags only when ≥3 rows already exist in `manager_analytics` for that jid. Fail-open on getChatTags error to prevent data loss on real clients.
- Default-tag new chats: in `upsertChat` (queries.js), after successful upsert check `getChatTags(remoteJid).tags.length === 0` and call `upsertChatTags(remoteJid, { tags:['неизвестно'], tagConfirmed:false })`. Skip groups (`@g.us`). Only triggers on the `else` branch (no upsert error) to avoid DB noise.
- `supabase.from('chat_tags').select('remote_jid').overlaps('tags', [...])` uses Postgres `&&` (array overlap) operator — correct for "has at least one of these tags". `.contains()` would require all tags to be present (wrong).
- Admin cleanup endpoints pattern: no extra middleware — the router-level authMiddleware already covers all routes including `/admin/*`. Use `.contains('tags', ['сотрудник'])` (not `.overlaps()`) for exact single-tag match on employee cleanup.
